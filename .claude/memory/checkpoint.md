...
## Фаза 22: Проверка коллизий целевого lcn; "Изменить lcn в модели" теперь меняет models, а не actives — ВЫПОЛНЕНА
- [x] Баг на `grom-my`: двухфазный `Z`-swap защищает только от цепочек ВНУТРИ файла, но не от коллизии с активом, которого в файле нет вообще — `UniqueViolationError` на `actives_lcn_key` прямо в середине выполнения. Добавлен `_check_lcn_collisions` (отдельный метод — использует `lcn::text`, не тестируется под SQLite) — батч-запросом сравнивает целевые `new_lcn` с уже существующими активами и, если владелец не входит в файл, выдаёт понятную построчную ошибку до выполнения. Вызывается для «Переместить активы без relocate» (актуально для actives). На `grom-tk` — 0 конфликтов (не ложных срабатываний), на `grom-my` — обнаружено и понятно описано 162 реальных конфликта из 360 пар (значит данные там уже частично мигрированы или структура отличается от grom-tk — решение по каждому требует ручного разбора, не автоматизировано)
- [x] Более серьёзная находка: «Изменить lcn в модели» ошибочно менял `actives.lcn`, хотя колонка `id` в файле (лист «Изменить LCN») — это `models.id` (та же схема, что у листов «Удалить из моделей»/«Добавить в модели» этого же файла), а `Старый lsn` совпадает с текущим `models.lcn` — подтверждено прямым запросом к `grom-tk`. Переписан полностью: `_validate_and_build_change_model_lcn_rows` (новый метод) резолвит по `models.id`, опционально сверяет `Старый lsn` с текущим `models.lcn` (защита от неверного файла), `_build_change_lcn_sql_lines` теперь двухфазный `UPDATE public.models` с matching по стабильному `id` (не по тексту lcn, в отличие от actives-варианта) — `lcn` входит в составной `UNIQUE(id_train_type, lcn, id_car_place, id_design_number, is_default)`, поэтому Z-swap всё равно нужен
- [x] «Переместить активы без relocate» не тронута — она и должна работать по активам (резолв через lsn/lcn, как "set serial=none lcn"); прежний общий метод `_validate_and_build_change_lcn_rows` переименован в `_validate_and_build_move_no_relocate_rows` для ясности (теперь однозначно принадлежит этой кнопке)
- [x] Тесты: `tests/test_parser_change_lcn_validation.py` → `tests/test_parser_move_no_relocate_validation.py` (actives-логика, без изменений в поведении); новый `tests/test_parser_change_model_lcn_validation.py` (11 тестов на models-логику — Postgres-only happy-path не покрыт под SQLite, тот же паттерн, что и везде на этой странице) — итого 143 теста зелёных
- [x] Проверено на `grom-tk` в откатанной транзакции: 72 строки листа «Изменить LCN» → 72 модели (1:1 с файлом, без экспансии по поездам — в отличие от actives-варианта, `models` не привязан к конкретным поездам), оба UPDATE-шага 72/72, `lcn` `M9.1.6→M9.1.6.4`, после `rollback()` вернулось обратно; отдельно проверены ошибки «Старый lsn не совпадает» и «модель не найдена»
- Коммиты: см. следующий коммит после этой записи

## Фаза 23: Парсинг ТМЦ — кнопка "Обновить unit_type" — ВЫПОЛНЕНА
- [x] Новая кнопка на странице «Парсинг ТМЦ» (`controllers/design_number_parser.py`), сделана по аналогии с уже существующей «Обновить id_counter_group»: `_validate_unit_type` (колонка `number` → `design_number.number`, `unit_type` → `unit_type.name`, без учёта регистра) + эндпоинты `/update-unit-type`, `/generate-sql-unit-type` — обновляет `design_number.id_unit_type`
- [x] Добавлена ORM-модель `UnitType` (`models.py`, таблица `public.unit_type`, только `id`/`name` — остальные колонки таблицы не нужны для этой задачи)
- [x] Синхронный флоу без прогресс-бара (как и остальные кнопки на этой странице — `counter-group`/`serial-1c`/`sync-counter-active`, в отличие от фонового progress-паттерна на странице «Парсинг моделей»); кнопка и ветки в общей модалке (`showModal`/`getEndpoints` в JS) добавлены по той же схеме, что и три существующих действия
- [x] Тесты: +6 (`tests/test_design_number_parser_validation.py`, хелпер `make_unit_type` в `tests/conftest.py`) — валидный ряд, регистронезависимость, пустые/неизвестные значения — итого 149 тестов зелёных
- [x] Проверено на реальном файле пользователя `Обновить unit_type.xlsx` (grom-tk): все 34 строки прошли валидацию без ошибок (`Ось колесной пары`→id 30, `Цельнокатное колесо`→id 31 и т.д.); в откатанной транзакции `design_number.id_unit_type` `None → 30` (uncommitted), после `rollback()` вернулось к `None`
- Коммиты: см. следующий коммит после этой записи

## Фаза 24: Парсинг моделей — комментарий с номером актива в SQL "Переместить активы" — ВЫПОЛНЕНА
- [x] `_build_move_actives_sql_body` (`controllers/parser.py`): строка `UPDATE public.actives SET ... WHERE id = {id_active};` теперь заканчивается SQL-комментарием `-- {active_number}` для читаемости сгенерированного файла (какой актив меняется на этой строке)
- [x] `active_number` — из БД (`_validate_and_build_move_rows`, `SELECT ... active_number FROM public.actives`), но на всякий случай переносы строк (`\n`/`\r`) заменяются на пробел перед подстановкой — иначе перенос строки внутри значения преждевременно закрыл бы `--`-комментарий и остаток строки исполнился бы как SQL
- [x] Тесты: `tests/test_parser_move_actives_validation.py` — существующие фикстуры дополнены `active_number`, ассерты проверяют наличие комментария; +1 новый тест на замену переносов строк — итого 150 тестов зелёных
- Коммиты: см. следующий коммит после этой записи

## Фаза 25: Парсинг моделей — кнопка "Изменить серийность в модели" (models.is_default) — ВЫПОЛНЕНА
- [x] Новая кнопка на странице «Парсинг моделей» (между «Изменить lcn в модели» и "set serial='none' lcn"): файл с колонками «id» (models.id) и «isdefault» (true/false/1/0/да/нет, как в `_validate_is_serial_1c`) — меняет `models.is_default` по id, без привязки к активам/поездам
- [x] `_validate_and_build_is_default_rows` — двухпроходный: 1) парсинг + дедуп по id (конфликт, если одна строка встречается дважды с разными значениями), не трогая default-множества; 2) проверка обоих частичных UNIQUE-индексов `models` (`WHERE is_default=true`: `(lcn, car_place)` и `(car_place, train_type, design_number)`, см. `_validate_and_build_rows`) — модели ИЗ ЭТОГО ЖЕ файла учитываются по новому значению из файла, а не по текущему в БД, иначе файл, одновременно снимающий старый default и ставящий новый на то же место, давал бы ложный конфликт из-за порядка строк
- [x] `_build_is_default_sql_lines`: отдельный `UPDATE public.models SET is_default = TRUE/FALSE WHERE id = ...;` на каждую строку, FALSE-строки сортируются перед TRUE — партиционный UNIQUE-индекс проверяется построчно по мере выполнения (не в конце транзакции), поэтому снятие старого default обязано выполниться раньше установки нового на то же место
- [x] Прогресс-бар (та же общая инфраструктура `_progress`/`_tasks`/`/parser/progress/{task_id}`, что и у "Изменить lcn в модели"/"set serial=none lcn"/"Переместить активы"): `/parser/is-default/{generate-sql,execute-sql}/start`; при выполнении прогресс обновляется на каждый отдельный UPDATE (не 0/1 как у батчевых кнопок) — при большом файле видно реальное движение, а не «зависшую» шкалу
- [x] Тесты: новый `tests/test_parser_is_default_validation.py` (18 тестов — парсинг колонок, дубликаты, оба UNIQUE-конфликта, сценарий "снять старый default + поставить новый на то же место в одном файле", сортировка SQL) — итого 168 тестов зелёных
- [x] Смоук: `litestar.testing.TestClient` — приложение стартует, `/parser/` маршрутизируется без конфликтов (полный happy-path с реальной БД не прогонялся в этой сессии)
- Входной файл пользователя `Исправление модели.xlsx`, лист «поменять isdefault» (колонки id/model/position/itemnum/description/lsn/isdefault) — прочитан для сверки реального формата колонок

## Фаза 26: Парсинг активов — кнопка "Создать актив из модели lcn" — ВЫПОЛНЕНА
- [x] Новая кнопка на странице «Парсинг активов» (`controllers/actives_parser.py`), по аналогии с «Создать активы из ТМЦ», но количество активов на позицию — НЕ из файла (нет колонки «Количество»), а количество поездов `public.train` с `id_train_type`, извлечённым из lsn (`M9.1.6.4` → 9); на каждый поезд создаётся отдельный актив
- [x] Нужные колонки файла: АРТИКУЛ (design_number.number), Тип актива (буквенный префикс active_number, как в create-actives), Серийный номер, lsn/lcn
- [x] `_parse_model_lcn` (regex `^\D*(\d+)(?:\.(.*))?$`) — та же логика, что и в `parser.py`, продублирована как модульная функция (контроллеры проекта самодостаточны, взаимных импортов друг у друга нет)
- [x] `_validate_create_active_from_model_rows`: id_car_place — `SELECT id_car_place FROM public.models WHERE id_train_type=... AND lcn::text=lsn` (полный lsn как есть, тот же `::text`-паттерн, что в train_parser.py при создании поезда); car_number — первый сегмент пути после id_train_type (`M9.1.6.4` → car_number=1); множественные разные id_car_place на один lcn — ошибка «неоднозначна»
- [x] location каждого актива: `id_type_location=2, id_train=<реальный id поезда>, car_number, id_car_place` (не storage-based, в отличие от create-actives) — car_number/id_car_place могут быть NULL
- [x] **Решение без явного подтверждения пользователя** (задавался уточняющий вопрос, ответа на два пункта не получено — применены рекомендованные по умолчанию варианты, стоит перепроверить на реальных данных):
  - если целевой lcn на конкретном поезде уже занят существующим активом — этот актив просто пропускается (не ошибка, не блокирует файл); батч-проверка одним запросом `lcn::text IN (...)` по всем вычисленным lcn файла + дедуп дублей внутри батча
  - серийный номер из файла применяется только если на позицию создаётся ровно 1 актив (у типа поезда всего 1 поезд), иначе всем активам `'none'` — тот же принцип, что и в `_validate_create_actives_rows`
- [x] `_build_create_active_from_model_sql_body`: DO-блок без складского счётчика (`storage.last_lcn` не нужен — lcn детерминирован моделью, не последовательный счётчик), только `iterator_number_last` под `FOR UPDATE` для номера актива
- [x] Эндпоинты `/create-active-from-model/{generate-sql,execute}/start`, общая прогресс-инфраструктура страницы (`_progress`/`_tasks`/`/actives-parser/progress/{task_id}`), кнопка + модалка + JS в `templates/actives_parser.html` переиспользуют общий `create-actives-progress-overlay`
- [x] Тесты: новый `tests/test_actives_parser_create_from_model_validation.py` (15 тестов — парсинг lsn, отсутствующие колонки/счётчик, пустой/длинный «Тип актива», ТМЦ не найдена, сборка SQL-тела: одиночный актив, NULL car_number/id_car_place/serial_number, инкремент счётчика на несколько активов) — итого 183 теста зелёных
- [x] Happy-path (реальный запрос к `models`/`train`/`actives` с `::text`-кастом ltree) не покрыт SQLite-тестами — тот же класс Postgres-only путей, что и `_check_lcn_collisions`/`_validate_and_build_change_model_lcn_rows`; не проверено на реальной БД в этой сессии (нет доступа на запись/rollback к `grom-tk` из текущего окружения)
- Входной файл пользователя `Исправление модели.xlsx`, лист «Создать активы» (колонки Артикул, Количество, Склад, Партия, Ячейка, Тип актива, Особый учет, Серийный номер, lsn) — прочитан для сверки формата; для этой кнопки используются только 4 колонки, остальные в файле не участвуют
- **Инцидент на реальной БД `192.168.92.143:6432/grom`**: запуск скачанного `create_active_from_model.sql` через pgAdmin создал 180 строк `actives` с `active_number IS NULL` (при этом `id_location`/`lcn`/`id_design_number` — корректны). Причина: `SELECT number INTO active_num FROM iterator_number_last ... FOR UPDATE;` в момент того конкретного запуска не нашёл строку счётчика (или она была пуста) — PL/pgSQL `SELECT ... INTO` без `STRICT` не ошибается на 0 строк, просто оставляет переменную NULL, и это NULL тихо протянулось через весь батч до `active_number`. Счётчик `iterator_number_last.number` на момент разбора уже был в порядке (1082719, не изменился — то есть финальный `UPDATE` в этом прогоне тоже не отработал/попал в тот же NULL) — точная причина именно того момента не установлена (не найдено расхождений в самом файле или в текущем состоянии БД при повторной проверке), но независимо от причины — отсутствие проверки было реальной дырой в надёжности
- Тот же паттерн (`SELECT ... INTO active_num ... FOR UPDATE` без проверки) уже существовал и в старом `_build_create_actives_sql_body` («Создать активы из ТМЦ») — не уникально для новой кнопки
- [x] Фикс (по требованию пользователя, в обоих местах): после чтения счётчика добавлена `IF active_num IS NULL THEN RAISE EXCEPTION 'Счётчик ''...'' не найден или пуст'; END IF;` — превращает тихий NULL в явную ошибку с откатом всей транзакции. Проверено на реальной БД в откатанных транзакциях: штатный путь (`active_num=1082719`) проходит, симуляция отсутствующего счётчика (`description='not-a-real-description'`) корректно даёт `RAISE EXCEPTION` и `ROLLBACK`
- 180 битых строк (`active_number IS NULL`) в БД пользователь решил не трогать/убрать сам — не удалялись в рамках этой сессии
- Обнаруженный попутно, но НЕ исправленный баг (по решению пользователя): `controllers/sql_console.py` (`_split_sql_statements`) не понимает dollar-quoting (`$$...$$`) и ломает любой вставленный в консоль `DO $$ ... END $$;`-скрипт на бессмысленные фрагменты — общая проблема для всех DO-блочных кнопок проекта, не только этой; для ручного запуска скачанного .sql предназначены psql/pgAdmin/кнопка «Выполнить в базе данных», не встроенная SQL-консоль приложения
- **Фикс #1** (ошибка «неоднозначна: несколько разных id_car_place [637, 12820]»): запрос дополнен `AND is_default = true`
- **Фикс #2** (регрессия от фикса #1 на реальной БД `192.168.92.143:6432/grom`: «Модель с lcn 'M9.2.5.4' не найдена», хотя `select * from models where lcn='M9.2.5.4'` даёт 2 строки) — обе строки models для этого lcn имели `is_default=false` одновременно (не гарантировано, что хотя бы одна строка default), при этом `id_car_place` у них совпадал (12903). Жёсткий фильтр `is_default=true` отфильтровывал вообще всё. Проверено на реальной БД (`psql` напрямую): и для 'M9.2.5.4' (обе строки non-default, car_place совпадает), и для 'M9.1.6.4' (одна default, одна нет, car_place тоже совпадает — 637) — `id_car_place` у всех строк одного lcn на практике идентичен. Логика переписана: сперва берутся все различные `id_car_place` по lcn БЕЗ фильтра по is_default; если значение одно (типичный случай) — используется сразу; `is_default=true` — тай-брейк, только если car_place реально разошёлся между строками; если и тогда неоднозначность не снята — ошибка по строке

## Фаза 26.1: Парсинг активов — короткое имя колонки "Позиция ТМЦ" — ВЫПОЛНЕНА
- [x] «Изменение ТМЦ номера»: колонка `Позиция ТМЦ` принята как третий алиас наравне с `Новая Позиция ТМЦ`/`Новый ТМЦ номер` (`_validate_design_number`) — реальный файл пользователя использовал именно короткое имя
- [x] Тесты: +1 (`test_design_number_short_column_name`) — итого 184 теста зелёных
- Коммит: 6adc5e9

## Фаза 27: Новая страница "Парсинг заказ нарядов" — присвоение родительского ЗН (order_to_order) — ВЫПОЛНЕНА
- [x] Новый контроллер `controllers/order_parser.py` (`/order-parser`), шаблон `templates/order_parser.html`, зарегистрирован в `app.py`; ссылка в сайдбаре (`base.html`) и карточка на «Главная» (`index.html`)
- [x] Добавлена ORM-модель `OrderToOrder` (`public.order_to_order`: `id_parent`, `id_child`, составной PK) — структура снята напрямую с реальной БД через `pg_attribute`/`pg_indexes` (в `information_schema` таблица не отдавала колонки — необъяснено, обошли через системные каталоги); подтверждён отдельный уникальный индекс на `id_child` — у одного заказ-наряда может быть только один родитель
- [x] Страница целиком по образцу «Парсинг ПТОиР» — фоновая генерация SQL/выполнение с прогресс-баром (`_progress`/`_tasks`, `/order-parser/progress/{task_id}`), кнопка «Присвоить родительский ЗН» открывает модалку (Скачать SQL-файл / Выполнить в базе данных), как «Запуск ПТОиР»
- [x] Файл: колонки «Заказ-наряд» (child) и «Родительский ЗН» (parent), обе резолвятся в `orders.id` по `orders.order_number` (уникальна в БД); валидация — пустые поля, самоссылка (ЗН = собственный родитель), дубликат «Заказ-наряд» внутри файла, ЗН не найден
- [x] SQL — не INSERT/UPDATE по отдельности, а один upsert на строку: `INSERT INTO public.order_to_order (id_parent, id_child) VALUES (...) ON CONFLICT (id_child) DO UPDATE SET id_parent = EXCLUDED.id_parent` — ключ конфликта соответствует реальному уникальному индексу `order_to_order_id_parent_uniq`
- [x] Тесты: новый `tests/test_order_parser_validation.py` (9 тестов) + хелпер `make_order` в `tests/conftest.py` — итого 193 теста зелёных
- [x] Смоук: приложение стартует с новым контроллером (`python -c "import app"`), `/order-parser` маршрутизируется (303 → логин, как и остальные защищённые страницы) на отдельном тестовом порту, не затрагивая уже запущенный процесс пользователя на 8011
- Входной файл пользователя `Присвоить Родительский ЗН (1) (1).xlsx` (135 строк, колонки «Заказ-наряд»/«Родительский ЗН») — прочитан для сверки формата
- Коммиты: см. следующий коммит после этой записи

## Фаза 28: Парсинг моделей — выбор "вставить без ошибок" при "Добавить строку в модель" → "Выполнить в базе данных" — ВЫПОЛНЕНА
- [x] Баг/пожелание: при ошибках валидации (в т.ч. «Дубликат: строка ... уже существует») кнопка «Выполнить в базе данных» просто блокировала вставку целиком — валидные строки не вставлялись, пока пользователь не почистит файл руками
- [x] `controllers/parser.py`: `_run_insert_execute` принимает `skip_errors: bool`; если есть ошибки и `skip_errors=False` — вместо `status="error"` (терминальная блокировка) отдаётся `status="confirm_errors"` с `errors` + `valid_count` (сколько строк прошло бы валидацию); при `skip_errors=True` ошибочные строки просто пропускаются, вставляются только `valid_rows`, итоговое сообщение показывает и вставлено, и пропущено с ошибками
- [x] `execute_sql_start` читает `skip_errors` из form-data (`"true"`/иначе)
- [x] `templates/parser.html`: `executeSQL(skipErrors)` — на `confirm_errors` показывает ошибки (как раньше) и доп. `confirm()` «Вставить N строк без ошибок?»; при согласии — повторный вызов `executeSQL(true)` с `skip_errors=true` в FormData; при отказе — ничего не отправляется, ошибки просто показаны (эквивалент отмены)
- [x] Область изменения — только «Добавить строку в модель» → «Выполнить в базе данных» (`/parser/execute-sql/start`); «Скачать SQL-файл» (`generate-sql/start`) и остальные 6 кнопок страницы не тронуты — пользователь просил именно про эту кнопку
- [x] Уточнение: пропущенные (не вставленные) строки должны выводиться в любом случае, а не только числом в alert — итоговый `status="done"` теперь тоже содержит `errors`; фронтенд при успешной вставке с пропусками рендерит `error-box`/подсветку строк (`handleErrors`) и НЕ делает `location.reload()` (иначе список ошибок тут же стирался бы) — reload остаётся только при полном успехе без ошибок
- [x] 193/193 теста зелёные (без новых тестов — изменение в фоновой orchestration-функции `_run_insert_execute`/JS, не в чистой валидационной логике; happy-path требует реальной БД, как и остальной execute-флоу на этой странице)
- Коммиты: см. следующий коммит после этой записи
